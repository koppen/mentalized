...
A bunch of people pointed to "this handy iTerm shortcut":http://railstips.org/2008/2/22/handy-iterm-shortcut, which uses Applescript invoked from a shell script to start the environment. I use roughly the same approach, however it's wrapped in "an Automator workflow":http://mentalized.net/files/journal/rails_project_launcher/EditRailsProjectWorkflow.zip, which I launch using "Quicksilver":http://docs.blacktree.com/quicksilver/what_is_quicksilver. 
...
It's a modified version of a workflow I found somewhere on the net. Unfortunately I cannot find the original source now, so I cannot even give credit where it's due. "Download it here":http://mentalized.net/files/journal/rails_project_launcher/EditRailsProjectWorkflow.zip.
