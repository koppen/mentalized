...
We'll cheat and use a few gems to do the heavy lifting for us. Namely the "to_xls gem":http://github.com/splendeo/to_xls that "transforms an Array into a excel file". Internally it relies on the using the "spreadsheet gem":http://spreadsheet.rubyforge.org/ that handles the actual Excel file for us.
...
bc(ruby). Mime::Type.register "application/vnd.ms-excel", :xls ActionController::Renderers.add :xls do |object, options|
